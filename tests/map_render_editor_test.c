    ...
    unsigned char enabled,float value,float min,float max,float step,float big,unsigned char input)
{
    ...
    assert(p==panel&&name&&label&&help&&input&&enabled&&step>0&&big>0&&value>=min&&value<=max);
    memset(v,0,0x140);v[0x28]=enabled;*(float*)(v+0x120)=value;return v;
}
...
int main(void)
{
    ...
    *(void**)(menu+0x9e0)=menu;g_installed=1;g_enter=native_state;g_exit=native_state;g_populate=native_populate;
    ...
    for(unsigned i=0;i<SH_RENDER_FIELDS;i++)assert(widgets[i][0x28]);
    *(float*)(widgets[1]+0x120)=80;assert(changed(panel,widgets[1],1));
    ...
    reset(editor+0x204d8);assert(!dirty(menu)&&widgets[0][0x28]&&g_draft.value[1]==0&&writes==0);
    ...
    *(float*)(widgets[1]+0x120)=99;changed(panel,widgets[1],1);leave(NULL,editor);
    ...
    leave(NULL,editor);open(map_a);assert(g_draft.value[1]==60);
    *(float*)(widgets[1]+0x120)=0;changed(panel,widgets[1],1);
    apply(editor+0x204d8);assert(saved_a.value[1]==0&&writes==2);
    leave(NULL,editor);open(map_a);assert(g_draft.value[1]==0&&widgets[1][0x28]);
